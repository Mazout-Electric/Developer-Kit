...
// ======================================================
// CAN SUPPORT — STM32 HAL FDCAN
//
// STM32G431 (STSPIN32G4) has one FDCAN peripheral: FDCAN1.
// PA11 = FDCAN1_RX, PA12 = FDCAN1_TX (AF9).
//
// HAL FDCAN is always available when building with STM32duino
// (stm32g4xx_hal_fdcan.h is part of the STM32G4 HAL package).
// No optional include guard needed.
// ======================================================
...
// ── HAL FDCAN init ──────────────────────────────────────────────────────────
// Clock assumption: PCLK1 = 170 MHz (STM32G431 default PLL at max speed).
// Bit timing for 500 kbps:
//   NominalPrescaler = 17  → CAN clock = 170/17 = 10 MHz
//   NominalSyncJumpWidth  = 1
//   NominalTimeSeg1       = 13  (Prop + Phase1)
//   NominalTimeSeg2       = 6
//   Total TQ per bit = 1 + 13 + 6 = 20 → 10 MHz / 20 = 500 kbps ✓
//
// FDCAN1 GPIO: PA11 (RX) / PA12 (TX) — Alternate Function 9
// ────────────────────────────────────────────────────────────────────────────
static bool fdcan1_init_ok = false;

static void fdcan1_gpio_init() {
...
static bool fdcan1_hal_init() {
  ...
  // STM32G4 HAL: message RAM sizing fields do NOT exist (unlike STM32H7).
  // The G4 FDCAN message RAM is shared and auto-partitioned by the HAL
  // based only on StdFiltersNbr / ExtFiltersNbr / TxFifoQueueMode.
  ...
  // Accept only std-ID frames matching rxCommandId() in RX FIFO0.
  // Use one std-ID filter. A second call to canSetup can reconfigure
  // the filter when device_id changes.
  FDCAN_FilterTypeDef sf = {};
  sf.IdType       = FDCAN_STANDARD_ID;
  sf.FilterIndex  = 0;
  sf.FilterType   = FDCAN_FILTER_MASK;
  sf.FilterConfig = FDCAN_FILTER_TO_RXFIFO0;
  sf.FilterID1    = rxCommandId();   // ID to match
  sf.FilterID2    = 0x7FF;           // mask: all bits must match
  HAL_FDCAN_ConfigFilter(&hfdcan1, &sf);

  // Reject all non-matching frames (don't fill FIFO with broadcast traffic)
...
// ── DLC byte-count lookup (STM32G4 HAL uses FDCAN_DLC_BYTES_n constants) ───
static uint32_t bytesToDLC(uint8_t n) {
  // FDCAN_DLC_BYTES_0..8 are defined as 0x00000000..0x00080000 in the G4 HAL.
  // For classic CAN frames we only ever send 0–8 bytes.
...
static uint8_t dlcToBytes(uint32_t dlc) {
  // Reverse: FDCAN_DLC_BYTES_n = n << 16 in STM32G4 HAL header
  uint8_t n = (uint8_t)((dlc >> 16) & 0x0F);
  return (n > 8) ? 8 : n;
}

// ── Thin TX helper ──────────────────────────────────────────────────────────
static int fdcan1_write(uint16_t id, const uint8_t *data, uint8_t len) {
  FDCAN_TxHeaderTypeDef h = {};
  h.Identifier          = id;
  h.IdType              = FDCAN_STANDARD_ID;
  h.TxFrameType         = FDCAN_DATA_FRAME;
  h.DataLength          = bytesToDLC(len);
  h.ErrorStateIndicator = FDCAN_ESI_ACTIVE;
  h.BitRateSwitch       = FDCAN_BRS_OFF;
  h.FDFormat            = FDCAN_CLASSIC_CAN;
  h.TxEventFifoControl  = FDCAN_NO_TX_EVENTS;
  h.MessageMarker       = 0;
  return HAL_FDCAN_AddMessageToTxFifoQ(&hfdcan1, &h, (uint8_t *)data);
}

// ── Thin RX helper ──────────────────────────────────────────────────────────
// Returns true and fills id/buf/len if a frame is waiting in FIFO0.
static bool fdcan1_read(uint16_t *id_out, uint8_t *buf, uint8_t *len_out) {

  if (HAL_FDCAN_GetRxFifoFillLevel(&hfdcan1, FDCAN_RX_FIFO0) == 0) return false;
  FDCAN_RxHeaderTypeDef rh = {};
  if (HAL_FDCAN_GetRxMessage(&hfdcan1, FDCAN_RX_FIFO0, &rh, buf) != HAL_OK) return false;
  *id_out  = (uint16_t)rh.Identifier;
  *len_out = dlcToBytes(rh.DataLength);
    Serial1.print(" data=");
    for (int i = 0; i < 8; i++) {
...
// ======================================================
void printCANStatus() {
  Serial1.print("CAN support : HAL FDCAN1 (PA11/PA12)");
  Serial1.print(fdcan1_init_ok ? " [OK]" : " [INIT FAILED]");
  Serial1.println();
  Serial1.print("DEV_ID      : "); Serial1.println(cfg.device_id);
  Serial1.print("RX CMD ID   : 0x"); Serial1.println(rxCommandId(), HEX);
  Serial1.print("TX STA ID   : 0x"); Serial1.println(txStatusId(),  HEX);
  Serial1.print("CAN runtime tx: "); Serial1.println(can_enable ? "ON" : "OFF");
}

void canSetup() {
  // If already running (e.g. called again after device ID change to
  // reprogram the RX filter), stop and deinit cleanly first.
  // Calling HAL_FDCAN_Init() on a running peripheral without stopping it
  // first will stall the peripheral and hang the bus.
  if (fdcan1_init_ok) {
    HAL_FDCAN_Stop(&hfdcan1);
    HAL_FDCAN_DeInit(&hfdcan1);
    fdcan1_init_ok = false;
  }
  fdcan1_init_ok = fdcan1_hal_init();
  if (fdcan1_init_ok) {
    Serial1.print("CAN HAL FDCAN1 init OK — PA11/PA12 @500kbps, DEVICE_ID=");
    Serial1.println(cfg.device_id);
  } else {
    Serial1.println("ERROR: CAN HAL FDCAN1 init FAILED");
  }
}

void canService() {
  if (!fdcan1_init_ok) return;

  // ── TX: periodic status frame ────────────────────────────────────────────
  if (can_enable && (millis() - last_can_tx >= 3000)) {
    last_can_tx = millis();
    ...
    uint8_t buf[8];
    buf[0] = pos_deg10  & 0xFF; buf[1] = (pos_deg10  >> 8) & 0xFF;
    buf[2] = vel_scaled & 0xFF; buf[3] = (vel_scaled >> 8) & 0xFF;
    buf[4] = tgt_deg10  & 0xFF; buf[5] = (tgt_deg10  >> 8) & 0xFF;
    buf[6] = cmd_scaled & 0xFF; buf[7] = (cmd_scaled >> 8) & 0xFF;

    int ok = fdcan1_write(txStatusId(), buf, 8);
    Serial1.println(ok);
  ...
  uint8_t  rxBuf[8];
  uint8_t  rxLen;
  while (fdcan1_read(&rxId, rxBuf, &rxLen)) {
    if (rxId != rxCommandId() || rxLen < 1) continue;
    uint8_t cmd = rxBuf[0];

    Serial1.println("Inside CAN Rx");

    if      (cmd == 0x01) {
      stopMotion();
    } else if (cmd == 0x02 && rxLen >= 3) {
      int16_t deg10 = (int16_t)(rxBuf[1] | (rxBuf[2] << 8));
      gotoOutputPositionRad(deg2radf(deg10 / 10.0f));
    } else if (cmd == 0x03 && rxLen >= 3) {
      int16_t deg10 = (int16_t)(rxBuf[1] | (rxBuf[2] << 8));
      ...
      stopMotion();
    } else if (cmd == 0x05 && rxLen >= 2) {
      uint8_t newId = rxBuf[1];
      if (newId >= 1 && newId <= 127) {
        cfg.device_id = newId;
        saveRuntimeToConfig();
        saveConfigToFlash();
        // Reconfigure RX filter to new command ID
        canSetup();
      }
    } else if (cmd == 0x06 && rxLen >= 3) {
      int16_t kp100 = (int16_t)(rxBuf[1] | (rxBuf[2] << 8));
      position_kp = kp100 / 100.0f; saveRuntimeToConfig(); saveConfigToFlash();
    } else if (cmd == 0x07 && rxLen >= 3) {
      int16_t ms100 = (int16_t)(rxBuf[1] | (rxBuf[2] << 8));
      position_max_speed = ms100 / 100.0f; saveRuntimeToConfig(); saveConfigToFlash();
    } else if (cmd == 0x08 && rxLen >= 3) {
      int16_t db10 = (int16_t)(rxBuf[1] | (rxBuf[2] << 8));
      position_deadband_rad = deg2radf(db10 / 10.0f); saveRuntimeToConfig(); saveConfigToFlash();
    } else if (cmd == 0x09 && rxLen >= 3) {
      int16_t vl100 = (int16_t)(rxBuf[1] | (rxBuf[2] << 8));
      motor.velocity_limit = vl100 / 100.0f;
      driver.voltage_limit = motor.voltage_limit;   // keep in sync
      saveRuntimeToConfig(); saveConfigToFlash();
    } else if (cmd == 0x0A) {
      saveRuntimeToConfig(); saveConfigToFlash();
    } else if (cmd == 0x0B && rxLen >= 2) {
      can_enable = (rxBuf[1] != 0); saveRuntimeToConfig(); saveConfigToFlash();
    }
  }
}
